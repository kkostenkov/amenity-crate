...
namespace Amenity.Utilities
{
    ...
    internal class MinMaxSliderDrawer : PropertyDrawer
    {
        ...
        {
            if (!this.pressed || (this.pressed && !Mathf.Approximately(min, this.pressedMin))) {
                using (var x = property.FindPropertyRelative(kVectorMinName)) {
                    SetValue(x, ref min, round);
                }
            }

            if (!this.pressed || (this.pressed && !Mathf.Approximately(max, this.pressedMax))) {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var fieldWidth = ppp * (attr.DataFields && attr.FlexibleFields
            ...
            if (this.pressed && Event.current.type == EventType.MouseUp) {
                if (attr.Round) {
